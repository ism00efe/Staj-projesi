...
from payment_assistant.models import Chunk
from payment_assistant.rag.engine import RAGEngine
from payment_assistant.rag.ingestion import chunk_document, load_corpus
from payment_assistant.rag.retriever import DenseRetriever
from payment_assistant.rag.sparse_retriever import build_bm25_from_corpus
from payment_assistant.sanitization import sanitize_text
...
def test_sparse_index_never_contains_raw_pii(tmp_path):
    ...
    bm25 = build_bm25_from_corpus(str(tmp_path), chunk_size=500, overlap=50)
...
def test_full_pipeline_query_containing_pii_never_leaks(tmp_path):
    ...
    for tag in ("[CARD]", "[TCKN]", "[EMAIL]", "[IP]", "[PHONE]", "[TOKEN]"):
